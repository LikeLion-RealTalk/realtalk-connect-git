...
export function DebatePage({ onNavigate, onGoBack, debateRoomInfo }: DebatePageProps) {
    ...
    }, [hasEnteredRoom, debateRoomInfo.userPosition, debateRoomInfo.id, fetchDebateRoomStatus]);
    ...
    // AI 요약 모달에서 나가기 요청 시 바로 토론방 둘러보기 페이지로 이동
    const handleAiSummaryExitRequest = () => {
        console.log('AI 요약 모달에서 나가기 요청됨 - 토론방 둘러보기로 이동');
        setIsAiSummaryModalOpen(false);

        // 모달이 완전히 닫힌 후 네비게이션 실행 (타이밍 이슈 해결)
        setTimeout(() => {
            console.log('네비게이션 실행: browser 페이지로 이동');
            onNavigate?.('browser');
        }, 100);
    };
    ...
    const generateAISummaryForSpeech = useCallback((speech: any) => {
        ...
        console.log('AI 요약 생성됨:', newSummary);
    }, [aiSummaries]);
        ...
        <>
            <div className={`h-screen flex flex-col overflow-hidden ${(isAiSummaryLoadingOpen || isAiSummaryModalOpen) && !isExitConfirmModalOpen ? 'debate-end-blur' : ''}`}>
                ...
                <div className="flex-1 flex h-[calc(100vh-64px)] lg:h-[calc(100vh-64px-48px)]">
                        ...
                        <div className="lg:hidden flex flex-col h-full">
                            ...
                            <div className="flex-1 bg-background overflow-hidden">
                                <Tabs defaultValue="speech" className="w-full h-full flex flex-col">
                                    ...
                                    {/* 탭 컨텐츠 */}
                                    <div className="flex-1 min-h-0">
                                        <TabsContent value="speech" className="h-full m-0 p-0">
                    ...
                    {/* 데스크톱 우측 사이드바 - 1/4 너비 */}
                    <div className="hidden lg:block lg:w-1/4 h-full flex flex-col">
                        {/* AI 요약 (상단 1/2) */}
            ...
            <DebateExtensionModal
                ...
                timeLeft={formatTime(debateTimeLeft)}
            />
