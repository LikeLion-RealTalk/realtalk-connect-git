...
export function PositionSelectionModal({
  ...
  const { isLoggedIn, user, nickname, setNickname: setGlobalNickname } = useUser();
  ...
  }, [isOpen, nickname, isLoggedIn, user, setGlobalNickname]);
  ...
  // 로그인한 사용자는 닉네임 검증 불필요, 비로그인 사용자는 닉네임 필수
  const canConfirm = selectedPosition && (isLoggedIn || localNickname.trim());
