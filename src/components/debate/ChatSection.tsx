...
// 콘텐츠 컴포넌트 분리
export function ChatSectionBody({ messages, onSendMessage, currentDebateStage, currentUserId }: ChatSectionProps) {
  ...
  // 발언자가 지정되고 발언 단계인지 확인 (대기중이 아닌 경우에만 비활성화)
  const isSpeechStage = currentUserId && currentDebateStage === '1. 발언';
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden relative">
        ...
        {isSpeechStage && (
          <div className="absolute inset-0 z-10 bg-white/80 backdrop-blur-sm flex items-center justify-center">
            <div className="text-center space-y-4">
              ...
              <div>
                <h3 className="text-lg font-medium text-gray-900 mb-2">발언중입니다</h3>
        ...
        <div 
          ...
          className={`flex-1 overflow-y-auto p-4 space-y-3 min-h-0 scrollbar-thin scrollbar-thumb-primary/20 scrollbar-track-transparent smooth-scroll ${isSpeechStage ? 'pointer-events-none' : ''}`}
        ...
        {/* 채팅 입력 */}
        <div className={`p-4 border-t border-divider flex-shrink-0 bg-surface elevation-1 ${isSpeechStage ? 'pointer-events-none' : ''}`}>
          <form onSubmit={(e) => { e.preventDefault(); handleSend(); }} className="flex gap-2">
            <Input
              ...
              autoComplete="off"
              disabled={isSpeechStage}
            />
            <Button 
              type="submit"
              disabled={!inputMessage.trim() || isSpeechStage}
...
// 기존 전체 컴포넌트 (데스크톱용)
export function ChatSection({ messages, onSendMessage, currentDebateStage, currentUserId }: ChatSectionProps) {
  return (
    <div className="h-full flex flex-col overflow-hidden">
      <ChatSectionHeader />
      <div className="flex-1 min-h-0">
        <ChatSectionBody messages={messages} onSendMessage={onSendMessage} currentDebateStage={currentDebateStage} currentUserId={currentUserId} />
      </div>
    </div>
  );
}