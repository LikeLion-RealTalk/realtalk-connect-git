...
export function CreateDiscussionModal({
  ...
  useEffect(() => {
    if (formData.debateType === "3분토론") {
      ...
      const threeMinuteDuration = getThreeMinuteDebateDuration(
          formData.maxSpeakers,
      );
      setFormData((prev) => ({
        ...prev,
        duration: threeMinuteDuration,
      }));
    } else {
  ...
  const handlePositionTextChange = (
      position: Position,
      value: string,
  ) => {
    if (position === "A입장") {
  ...
  const getCurrentPositionText = () => {
    return formData.position === "A입장"
        ? formData.aPosition
        : formData.bPosition;
  };

  const handleSubmit = () => {
    // 필수 필드 검증 - 제목과 카테고리만 필수
    if (
        !formData.title.trim() ||
        !formData.category
    ) {
    ...
    // A입장, B입장 미입력 시 기본값 설정
    const finalFormData = {
      ...formData,
      aPosition: formData.aPosition.trim() || "찬성",
      bPosition: formData.bPosition.trim() || "반대"
    };
            ...
            <div className="flex-1 overflow-y-auto bg-background">
              <div className="space-y-6 p-4">
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-xs text-muted-foreground">
                    토론에 참여할 입장을 선택하세요. 입장 내용을 입력하지 않으면
                    기본값(찬성/반대)으로 설정됩니다.
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <RadioGroup
                      ...
                      onValueChange={(value) =>
                          handleSpeakersChange(parseInt(value))
                      }
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    {formData.duration}분
                      {formData.debateType === "3분토론" &&
                          ` (${formData.maxSpeakers}명 × 1분)`}
                  </span>
                  </div>
                  ...
                  {formData.debateType === "일반토론" ? (
                      <div className="space-y-2">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {formData.maxSpeakers}명의 발언자로 인해
                          최소 {minDuration}분부터 선택 가능합니다.
                        </p>
                      </div>
                  ) : (
                      <div className="text-sm text-muted-foreground">
                        3분 토론은 발언자 수에 따라 자동으로 시간이
                        설정됩니다.
                      </div>
                  )}
                </div>
          ...
          <div className="flex-1 overflow-y-auto px-1">
            <div className="space-y-6 py-4">
              ...
              <div className="space-y-4">
                ...
                <p className="text-xs text-muted-foreground">
                  토론에 참여할 입장을 선택하고 구체적인 입장을
                  입력해주세요. 입력한 내용은 입장 변경 시에도
                  유지됩니다.
                </p>
              </div>
